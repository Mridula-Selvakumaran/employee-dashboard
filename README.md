...
The results page pulls that image back out and shows it alongside an SVG bar chart and a Leaflet map. The chart is built entirely from `<rect>` and `<text>` SVG elements that is no Chart.js, no D3. The map uses a hardcoded dictionary to look up city coordinates since all the cities in the dataset are known. The verified employee's city gets highlighted in red on both the chart and the map, and their location marker opens automatically.
